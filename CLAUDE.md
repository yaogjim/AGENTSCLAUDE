...
本目录存放多项目共用的 agent 规范**母版**：同目录下的 `AGENTS.md` 是通用模板，仅供拷贝到各项目根目录后适配使用，**不对本目录下的任何子项目直接生效**。各项目一律以其自己根目录的 `AGENTS.md` / `CLAUDE.md` 为准；若本文件与子项目规范同时被加载，忽略本文件。

> 注意：本文件**刻意不使用** `@AGENTS.md` 引用——Claude Code 会向上递归加载父目录的 CLAUDE.md，若在此引用母版，会把通用模板（含未适配的占位路径）注入所有子项目会话，与项目自身规范冲突。

## 新项目接入步骤

1. 拷贝本目录 `AGENTS.md` 到新项目根目录，按其顶部「适配清单」逐项完成项目化替换，并选择唯一活动任务真值源；默认使用 `task/todo.md`，只有明确需要多 agent 并行认领时才使用 `todos/`。
...
4. 确认两个文件被 git 跟踪（不要放进 `.gitignore`），保留规范演进历史。
5. 母版更新后，用 diff 同步到各项目，注意区分「项目适配区」的刻意差异与意外漂移。
